...
(function (factory) {
	...
	function core(options, callback) {
		...
		function setImage(image) {
			...
			// }
			_IMG_.onload = function () {
				if (_IMG_.width === 0 || _IMG_.height === 0) {
					console.error("⛔ Image loaded but has no size.");
					return;
				}
				initialize();
			};
			...
			AdaptImg(originDetails, $_CONTAINER, function (ADAPTED) {
				...
				Cropper(originDetails, ADAPTED, function (CROPPED) {
					...
					setTimeout(() => {
						...
						if (_IMG_.width > 0 && _IMG_.height > 0) {
							ctx.drawImage(_IMG_, 0, 0, _IMG_.width, _IMG_.height);
						} else {
							console.error("⛔ Image not loaded properly. Width/Height is 0.");
							return;
						}
					}, 10)
